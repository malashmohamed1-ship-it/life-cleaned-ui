...
export default function ChatUI() {
  // messages: [{ id, role: "user"|"assistant", content, feedbackDone?: boolean }]
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const aiMsg = {
        ...
        role: "assistant",
        content: answer,
        // keep feedback state per assistant message
        feedbackDone: false,
        // store the prompt paired with this answer for feedback record
        pairedPrompt: prompt,
      };
  ...
  const markFeedbackSubmitted = (id) => {
    setMessages((m) =>
      m.map((msg) =>
        msg.id === id ? { ...msg, feedbackDone: true } : msg
      )
    );
  };
      ...
      <div className="flex-1 overflow-y-auto pr-1">
        ...
        {messages.map((msg, i) => (
          <div key={msg.id}>
            ...
            {/* Feedback only under assistant messages */}
            {msg.role === "assistant" && (
